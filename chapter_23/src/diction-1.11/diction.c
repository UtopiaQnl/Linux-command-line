...
static void loadPhrases(const char *file) /*{{{*/
{
    ...
    while (fgets(ln, sizeof(ln), fp)) {
        ...
        if (ln[0]) {
            ...
            {
                if ((badPhrases = realloc(
                         badPhrases, (badPhraseCapacity = 3 * (badPhraseCapacity + 32)) * sizeof(struct badPhrase))) ==
                    (struct badPhrase *)0) {
...
static void diction(const char *sent, size_t length, const char *file, int line) /*{{{*/
{
    ...
    while (s < end) {
        ...
        for (j = 0; j < badPhraseSize; ++j) {
            ...
            str = s;
            while ((*badword == tolower(*str) || *badword == *str) && *badword && *str) {
                ++badword;
                ++str;
            }
        ...
        if (doubleWords) {
            ...
            if (s > sent && !isalpha(*(s - 1))) {
                ...
                badword = s - 1;
                while (badword > sent && !isalpha(*badword))
                    --badword;
                if (badword > sent) {
                    ...
                    str = s;
                    while (*badword == *str && badword < s && isalpha(*str)) {
                        ++badword;
                        ++str;
                    }
...
int main(int argc, char *argv[]) /*{{{*/
{
    ...
    while ((c = getopt_long(argc, argv, "bdsf:nL:qh", lopts, (int *)0)) != EOF)
        switch (c) {
        case 'b':
            beginner = 1;
            break;
        ...
        case 'n':
            ...
            break;
        case 'L':
            strncpy(phraseLanguage, optarg, sizeof(phraseLanguage) - 1);
            phraseLanguage[sizeof(phraseLanguage) - 1] = '\0';
            break;
        case 'q':
            quiet = 1;
    ...
    hits = 0;
    if (optind == argc)
        sentence("diction", stdin, "(stdin)", diction, phraseLanguage);
    else
        while (optind < argc) {
            FILE *fp;
    ...
    if (sentences == 0) {
        ...
        if (hits == 0)
            ...
            printf(_("%d phrases "), hits);
        if (sentences == 1)
            printf(_("in 1 sentence found.\n"));
        else
            printf(_("in %d sentences found.\n"), sentences);
    }
    exit(0);
}
/*}}}*/
